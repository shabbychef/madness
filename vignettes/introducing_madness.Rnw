...
represent \Sexpr{nrow(ffret)} weeks of data, from 
\Sexpr{ffret[1,]$Date} to 
\Sexpr{ffret[nrow(ffret),]$Date}. \cite{Quandl}

<<'sr_demo_1',echo=TRUE,cache=TRUE>>=
ff3 <- ffret[,c('Mkt.RF','SMB','HML')]
...
\StockTicker{\Sexpr{colnames(allweekly)[ncol(allweekly)-1]}} and of
\StockTicker{\Sexpr{colnames(allweekly)[ncol(allweekly)]}}, collected over
\Sexpr{nrow(allweekly)} weeks from 
\Sexpr{allweekly[1,]$Date} to 
\Sexpr{allweekly[nrow(allweekly),]$Date}, as downloaded from Quandl. \cite{Quandl}
...
tht <- theta(allweekly %>% select(AAPL,IBM,Mkt.RF,SMB,HML) %>% mutate(one=1.0),xtag='stocks')
...
\StockTicker{\Sexpr{colnames(allweekly)[ncol(allweekly)-1]}} and 
\StockTicker{\Sexpr{colnames(allweekly)[ncol(allweekly)]}} discussed above\footnote{It should be
...
library(sandwich)
twom <- twomoments(allweekly %>% select(AAPL,IBM,Mkt.RF,SMB,HML),vcov=sandwich::vcovHAC,diag.only=FALSE)
...
library(sandwich)
tht <- theta(allweekly %>% mutate(one=1.0) %>% select(one,AAPL,IBM,Mkt.RF,SMB,HML),
