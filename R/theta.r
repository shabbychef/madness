...
#' @title Estimate the symmetric second moment matrix of values.
#'
#' @description 
#'
...
#' @export
#' @rdname theta
theta <- function(X,vcov.func=vcov) {
	X <- na.omit(X)
	if (is.data.frame(X)) {
		X <- as.matrix(X)
	}
