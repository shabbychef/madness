...
for (cnum in seq_len(ncol(xy))) {
	xydup[,(cnum-1) * ncol(xy) + (1:ncol(xy))] <- (as.numeric(xy[,cnum,drop=TRUE]) * as.matrix(xy))
}
xymean <- colMeans(xydup)
xycov <- cov(xydup)
# put it in a madness object
theta <- madness(xymean,ytag='xandy',xtag='xandy',varx=xycov)
...
stat=as.numeric(c(HLT,PBT,WLRT,RLR)),
Wald.stat=as.numeric(c(Hwald,Pwald,Uwald,Rwald)))
# rut-roh! 
kable(preso)
```
